...
function AboutPanel({ onCreate }) {
  return (
    <div style={{width:'100%',maxWidth:560,textAlign:'center',padding:'3rem 1rem 5rem'}}>
      ...
      <div style={{fontFamily:'Inter',fontSize:44,fontWeight:800,letterSpacing:'-0.025em',color:COLORS.text,marginBottom:10}}>SatoshiLock</div>
      <div style={{fontSize:14,color:COLORS.textDim,lineHeight:1.65,marginBottom:24,maxWidth:440,margin:'0 auto 24px'}}>
        Lock and distribute tokens over time on Solana. Open source, audited, non-custodial.
      </div>
      ...
      <button style={{...S.primaryCta, width:'auto', padding:'11px 26px', margin:'0 auto'}} onClick={onCreate}>Create lock</button>
    </div>
  );
}
    ...
    <div style={{width:'100%',maxWidth:780}}>
      ...
      {publicKey && rows.length === 0 && !refreshing && (
        <div style={{padding:'3rem 1rem',textAlign:'center'}}>
          ...
          <button style={{...S.primaryCta, width:'auto', padding:'10px 22px', margin:'0 auto'}} onClick={onCreate}>Create lock</button>
        </div>
      )}
...
const S = {
  ...
  connectBtn: { background:'transparent', border:`1px solid ${COLORS.accent}`, color:COLORS.accent, padding:'7px 16px', borderRadius:8, fontSize:13, fontWeight:700, cursor:'pointer', fontFamily:'inherit', transition:'all 0.2s' },
  walletPill: { display:'flex', alignItems:'center', gap:6, background:COLORS.cardBgDarker, border:`1px solid ${COLORS.border}`, borderRadius:8, padding:'7px 12px', color:COLORS.text, fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:'inherit' },
